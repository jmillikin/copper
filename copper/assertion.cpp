...
{
	EXPORT
	Assertion::Assertion (const AssertionResult &result,
	                      const String &text,
	...
	EXPORT
	Assertion::Assertion (bool, int) throw ():
	                      m_result (true),
	                      m_text (""),
	                      m_line (0)
	{
	...
	EXPORT
	const String &
	Assertion::failure_message () const throw ()
	{
		if (m_message.size ())
			return m_message;
	...
	failed_func (const Copper::Assertion &assertion) throw ()
	{
		...
		{
			String message = String ("Unexpected success of assertion '") + assertion.text () + "'";
			result.fail (message);
		}
